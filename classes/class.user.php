...
if(!class_exists('Auto_User')) {
	...
	class Auto_User extends WP_User {
		...
		function send_message( $subject, $message ) {
			...
			if(!empty($this->user_email)) {
				...
				if(!empty($message)) {
					...
					$msg .= "\n\n"; // Two blank lines
					$msg .= "-----\n"; // Footer marker
					$msg .= __('Um den Empfang von Nachrichten von %sitename% zu beenden, klickeSie auf den folgenden Link: %siteurl%/unsubscribe/','automessage');
					// Add in the user id
		...
		function send_unsubscribe() {
			...
			if($from_url)
				$header = 'From: "' . $replacements['/%sitename%/'] . '" <noreply@' . $from_url . '>';
			$res = @wp_mail( $this->user_email, __("Abmeldeanfrage bearbeitet", 'automessage'), __("Deine Abmeldeanfrage wurde bearbeitet und Du wurdest aus unserer Mailingliste entfernt.\n\nVielen Dank\n\n", 'automessage'), $header );
			return $res;
		}
