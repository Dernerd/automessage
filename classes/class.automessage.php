...
class automessage {
	...
	function initialise_plugin() {
		...
		register_post_type('automessage', array(	'singular_label' => __('Automessage','automessage'),
													'label' => __('Automessage', 'automessage'),
													'public' => false,
													'show_ui' => false,
	...
	function output_admin_notices() {
		...
		if(isset( $_GET['automessagemsg'] )) {
			switch( $_GET['automessagemsg'] ) {
				...
				case 1:		echo '<div id="message" class="updated fade"><p>' . __('Benutzer zur Automessage-Warteschlange hinzugefügt.', 'automessage') . '</p></div>';
							break;

				case 2:		echo '<div id="message" class="error"><p>' . __('Der Benutzer konnte nicht zur Automessage-Warteschlange hinzugefügt werden.', 'automessage') . '</p></div>';
							break;

				case 3:		echo '<div id="message" class="updated fade"><p>' . __('Blog-Administrator zur Automessage-Warteschlange hinzugefügt.', 'automessage') . '</p></div>';
							break;

				case 4:		echo '<div id="message" class="error"><p>' . __('Blog-Administrator konnte nicht zur Automessage-Warteschlange hinzugefügt werden.', 'automessage') . '</p></div>';
							break;
			}
		}
	...
	function add_user_to_queue_action( $actions, $user_object ) {
		...
		if(!$user->on_action()) {
			$actions['automessage'] = "<a class='submitautomessage' href='" . wp_nonce_url( $url."action=addtoautomessageuserqueue&amp;user=$user_object->ID", 'queueuser' ) . "' title='" . __('Benutzer zur Automessage-Warteschlange hinzufügen', 'automessage') . "'>" . __( 'Warteschlange', 'automessage' ) . "</a>";
		}
	...
	function add_blog_to_queue_action( $actions, $blog_id, $blog_name ) {
		...
		if($user_id !== false) {
			...
			if(!$user->on_action( 'blog' )) {
				$actions['automessage'] = '<a href="' . $delete = esc_url( network_admin_url( add_query_arg( '_wp_http_referer', urlencode( stripslashes( $_SERVER['REQUEST_URI'] ) ), wp_nonce_url( 'sites.php', 'queueblog' ) . '&amp;action=addtoautomessageblogqueue&amp;id=' . $blog_id ) ) ) . '" class="submitautomessage" title="' . __('Blog zur Automessage-Warteschlange hinzufügen', 'automessage') . '">' . __( 'Warteschlange', 'automessage' ) . '</a>';
			}
		}
	...
	function setup_menu() {
		...
		if(is_multisite() && function_exists('is_super_admin') && is_super_admin() && $blog_id == 1) {
			add_submenu_page('automessage', __('Blog-Nachrichten bearbeiten','automessage'), __('Nachrichten auf Blog-Ebene','automessage'), 'edit_automessage', "automessage_blogadmin", array(&$this,'handle_blogmessageadmin_panel'));
		}

		add_submenu_page('automessage', __('Benutzernachrichten bearbeiten','automessage'), __('Nachrichten auf Benutzerebene','automessage'), 'edit_automessage', "automessage_useradmin", array(&$this,'handle_usermessageadmin_panel'));
	...
	function show_admin_messages() {
		...
		if(isset($_GET['msg'])) {
			...
			switch($msg) {
				case 1:		echo '<div id="message" class="updated fade"><p>' . __('Deine Aktion wurde dem Zeitplan hinzugefügt.', 'automessage') . '</p></div>';
							break;

				case 2:		echo '<div id="message" class="updated fade"><p>' . __('Deine Aktion konnte nicht hinzugefügt werden.', 'automessage') . '</p></div>';
							break;

				case 3:		echo '<div id="message" class="updated fade"><p>' . __('Die geplante Aktion wurde angehalten', 'automessage') . '</p></div>';
							break;

				case 4:		echo '<div id="message" class="updated fade"><p>' . __('Die geplante Aktion wurde unterbrochen', 'automessage') . '</p></div>';
							break;

				case 5:		echo '<div id="message" class="updated fade"><p>' . __('Bitte wähle eine Aktion zum Löschen aus', 'automessage') . '</p></div>';
							break;

				case 6:		echo '<div id="message" class="updated fade"><p>' . __('Die geplanten Aktionen wurden pausiert', 'automessage') . '</p></div>';
							break;

				case 7:		echo '<div id="message" class="updated fade"><p>' . __('Bitte wähle eine Aktion zum Pausieren aus', 'automessage') . '</p></div>';
							break;

				case 8:		echo '<div id="message" class="updated fade"><p>' . __('Die geplanten Aktionen wurden angehalten', 'automessage') . '</p></div>';
							break;

				case 9:		echo '<div id="message" class="updated fade"><p>' . __('Bitte wähle eine Aktion aus, um die Pause aufzuheben', 'automessage') . '</p></div>';
							break;

				case 10:	echo '<div id="message" class="updated fade"><p>' . __('Die geplanten Aktionen wurden verarbeitet', 'automessage') . '</p></div>';
							break;

				case 11:	echo '<div id="message" class="updated fade"><p>' . __('Bitte wähle eine Aktion zur Verarbeitung aus', 'automessage') . '</p></div>';
							break;

				case 12:	echo '<div id="message" class="updated fade"><p>' . __('Die geplante Aktion wurde gelöscht', 'automessage') . '</p></div>';
							break;

				case 13:	echo '<div id="message" class="updated fade"><p>' . __('Die geplante Aktion wurde aktualisiert', 'automessage') . '</p></div>';
							break;

				case 14:	echo '<div id="message" class="updated fade"><p>' . __('Die geplante Aktion wurde verarbeitet', 'automessage') . '</p></div>';
							break;
	...
	function edit_action_form($id, $type) {
		...
		echo "<div class='wrap'>";
		echo "<h2>" . __('Aktion bearbeiten', 'automessage') . "</h2>";

		echo '<div id="poststuff" class="metabox-holder">';
		?>
		<div class="postbox">
			<h3 class="hndle" style='cursor:auto;'><span><?php _e('Aktion bearbeiten','automessage'); ?></span></h3>
			<div class="inside">
		<?php
		...
		echo '<th style="" scope="row" valign="top">' . __('Aktion','automessage') . '</th>';
		echo '<td valign="top">';
		...
		echo '<select name="hook" style="width: 40%;">';
			switch($type) {
				case 'blog':	echo '<option value="wpmu_new_blog"';
								echo '>';
								echo __('Neuen Blog erstellt','automessage');
								echo '</option>';
								break;

				case 'user':	echo '<option value="wpmu_new_user"';
								echo '>';
								echo __('Neuer Benutzer erstellt','automessage');
								echo '</option>';
								break;
			}
		echo '</select>';
		...
		echo '<tr class="form-field form-required">';
		echo '<th style="" scope="row" valign="top">' . __('Nachrichtenverzögerung','automessage') . '</th>';
		echo '<td valign="top">';
		...
		for($n = 0; $n <= AUTOMESSAGE_POLL_MAX_DELAY; $n++) {
			...
			echo ">";
			switch($n) {
				case 0: 	echo __("Sofort senden", 'automessage');
							break;
				case 1: 	echo __("1 Tag", 'automessage');
							break;
				default:	echo sprintf(__('%d Tage','automessage'),$n);
			}
			echo "</option>";
		}
		echo '</select>';
		...
		echo '<tr class="form-field form-required">';
		echo '<th style="" scope="row" valign="top">' . __('Betreff der Nachricht','automessage') . '</th>';
		echo '<td valign="top"><input name="subject" type="text" size="50" title="' . __('Betreff der Nachricht') . '" style="width: 50%;" value="' . htmlentities(stripslashes($editing->post_title),ENT_QUOTES, 'UTF-8') . '" /></td>';
		echo '</tr>';

		echo '<tr class="form-field form-required">';
		echo '<th style="" scope="row" valign="top">' . __('Nachricht','automessage') . '</th>';
		...
		echo __('Du kannst die folgenden Konstanten im Nachrichtentext verwenden, um Datenbankinformationen einzubetten.','automessage');
		echo '<br /><br />';
		echo '%blogname%<br />';
		echo '%blogurl%<br />';
		echo '%firstname%<br/>';
		...
		echo '<p class="submit">';
		echo '<input class="button-primary" type="submit" name="go" value="' . __('Aktion aktualisieren', 'automessage') . '" /></p>';
		echo '</form>';
	...
	function add_action_form($type) {
		...
		echo "<h2>" . __('Aktion hinzufügen', 'automessage') . "</h2>";
		...
		?>
		<div class="postbox">
			<h3 class="hndle" style='cursor:auto;'><span><?php _e('Aktion hinzufügen','automessage'); ?></span></h3>
			<div class="inside">
		<?php
		...
		echo '<select name="hook" style="width: 40%;">';
			switch($type) {
				...
				case 'user':	echo '<option value="wpmu_new_user">';
								echo __('Neuen Benutzer erstellt','automessage');
								echo '</option>';
								break;
			}
		echo '</select>';
		...
		echo '<tr class="form-field form-required">';
		echo '<th style="" scope="row" valign="top">' . __('Betreff der Nachricht','automessage') . '</th>';
		echo '<td valign="top"><input name="subject" type="text" size="50" title="' . __('Betreff der Nachricht') . '" style="width: 50%;" /></td>';
		echo '</tr>';
		...
		echo '<p class="submit">';
		echo '<input class="button-primary" type="submit" name="go" value="' . __('Aktion hinzufügen', 'automessage') . '" /></p>';
		echo '</form>';
	...
	function show_actions_list($results = false, $type = 'user') {
		...
		echo '<th scope="col">';
		echo __('Aktion','automessage');
		echo '</th>';

		echo '<th scope="col">';
		echo __('Zeitverzögerung','automessage');
		echo '</th>';

		echo '<th scope="col">';
		echo __('Betreff','automessage');
		echo '</th>';

		echo '<th scope="col">';
		echo __('Warteschlange','automessage');
		echo '</th>';
		...
		if($results) {
			...
			foreach($results as $result) {
				...
				if($lasthook != $hook) {
					switch($hook) {
						case 'wpmu_new_blog':	$title = __('Neuen Blog erstellt','automessage');
												break;

						case 'wpmu_new_user':	$title = __('Neuen Benutzer erstellt','automessage');
												break;
					}
				...
				if($result->post_status == 'draft') {
					echo __('[Angehalten] ','automessage');
				}
				echo apply_filters( 'automessage_row_action_title', $title, $hook );

				$actions = array();

				$actions[] = '<a href="?page=' . $page . '&amp;action=editaction&amp;id=' . $result->ID . '" title="' . __('Diese Nachricht bearbeiten','automessage') . '">' . __('Bearbeiten','automessage') . '</a>';
				if($result->post_status == 'private') {
					$actions[] = '<a href="?page=' . $page . '&amp;action=pauseaction&amp;id=' . $result->ID . '" title="' . __('Pausiere diese Nachricht','automessage') . '">' . __('Pause','automessage') . '</a>';
				} else {
					$actions[] = '<a href="?page=' . $page . '&amp;action=unpauseaction&amp;id=' . $result->ID . '" title="' . __('Forstetzen dieser Nachricht','automessage') . '">' . __('Forstetzen','automessage') . '</a>';
				}
				$actions[] = '<a href="?page=' . $page . '&amp;action=process' . $type . 'action&amp;id=' . $result->ID . '" title="' . __('Verarbeite diese Nachricht','automessage') . '">' . __('Verarbeiten','automessage') . '</a>';
				$actions[] = '<a href="?page=' . $page . '&amp;action=deleteaction&amp;id=' . $result->ID . '" title="' . __('Diese Nachricht löschen','automessage') . '">' . __('Löschen','automessage') . '</a>';
				...
				if($result->menu_order == 0) {
					echo __('Sofortig','automessage');
				} elseif($result->period == 1) {
					echo sprintf(__('%d %s','automessage'), $result->menu_order, 'day');
				} else {
					echo sprintf(__('%d %se','automessage'), $result->menu_order, 'day');
				}
			...
			echo '<tr>';
			echo '<td colspan="5">' . __('Keine Aktionen für diese Ebene festgelegt.') . '</td>';
			echo '</tr>';
		}
	...
	function dashboard_news() {
		...
		<div class="postbox ">
			...
			<div class="inside">
				<?php
				echo "<p>";
				echo __('Du verwendest die Automessage-Version ','automessage') . "<strong>" . $plugin['Version'] . '</strong>';
				echo "</p>";

				echo "<p>";
				echo __('Debug-Modus ist ','automessage') . "<strong>";
				if($debug) {
					echo __('aktiviert','automessage');
				} else {
					echo __('deaktiviert','automessage');
				}
				echo '</strong>';
				echo "</p>";
				?>
	...
	function handle_blogmessageadmin_panel() {
		...
		echo "<h2>" . __('Nachrichten auf Blog-Ebene','automessage');
		echo '<a class="add-new-h2" href="' . remove_query_arg('msg', add_query_arg(array('action' => 'newaction'))) . '">' . __('Neue hinzufügen','automessage') . '</a>';
		echo "</h2>";
		...
		echo '<input type="submit" value="' . __('Löschen','automessage') . '" name="allaction_delete" class="button-secondary delete" style="margin-right: 10px;" />';
		echo '<input type="submit" value="' . __('Pause','automessage') . '" name="allaction_pause" class="button-secondary" style="margin-right: 10px;" />';
		echo '<input type="submit" value="' . __('Fortsetzen','automessage') . '" name="allaction_unpause" class="button-secondary" />';
	...
	function handle_usermessageadmin_panel() {
		...
		echo "<div class='wrap'  style='position:relative;'>";
		echo '<div class="icon32" id="icon-edit-pages"><br></div>';
		echo "<h2>" . __('Nachrichten auf Benutzerebene','automessage');
		echo '<a class="add-new-h2" href="' . remove_query_arg('msg', add_query_arg(array('action' => 'newaction'))) . '">' . __('Neue hinzufügen','automessage') . '</a>';
		echo "</h2>";
		...
		if(!empty($users) && $lastprocessing <= strtotime('-30 minutes')) {
			...
			foreach( (array) $users as $user_id) {
				...
				if(time() > $timestart + $timelimit) {
					if($this->debug) {
						// time out
						$this->errors[] = sprintf(__('Hinweis: Verarbeitung wegen %d zweiter Zeitüberschreitung gestoppt.','automessage'), $timelimit);
					}
					break;
				}
	...
	function output_unsubscribe_message() {
		...
		$post->post_title = __('Abmeldeantrag','automessage');
		$post->post_content = '<p>'.__('Deine Abmeldeanfrage wurde erfolgreich bearbeitet.','automessage').'</p>';
		$post->post_excerpt = __('Deine Abmeldeanfrage wurde erfolgreich bearbeitet.','automessage');
		$post->ID = -1;
